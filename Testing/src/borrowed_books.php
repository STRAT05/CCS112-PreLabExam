...
$sql = "SELECT * FROM books WHERE status='Borrowed' ORDER BY book_id DESC";
...
<body>
    ...
    <form method="POST" action="return_multiple.php">
        <table>
            <tr>
                <th>
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td>
                            <label>
                                <input type="checkbox" class="book-checkbox" name="selected_books[]" value="<?= $row['book_id'] ?>">
                                Return
                            </label>
                        </td>
                        <td><?= $row['book_id'] ?></td>
